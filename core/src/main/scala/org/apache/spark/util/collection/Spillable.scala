...
private[spark] abstract class Spillable[C](taskMemoryManager: TaskMemoryManager)
  ...
  private[this] val recordsCheckInterval: Long =
    SparkEnv.get.conf.getLong("spark.shuffle.spill.recordsCheckInterval", 32)
  ...
  @inline private def logSpillage(size: Long) {
    ...
    logInfo("[Spill] Thread %d spilling in-memory map of %s to disk (%d time%s so far)"
